/*!
  ...
  \brief Reads GPS NMEA messages, writes to cloud queue.
...
int gps_pos_changed( struct gps_pos_t *gps_pos ) {
  if (
      ( fabs(gps_pos->lat[0] - gps_pos->lat[1]) <= 0.0001 )
      &&
      ( fabs(gps_pos->lon[0] - gps_pos->lon[1]) <= 0.0001 )
      ) {
    return 0;
  }
  return 1;
	...
	if ( strlen(ans) == 0 ) {
	  ...
	  */
	  ...
	  if ( gps_pos.fix == 0 ) { // for testing
	    ...
	    printf( "%s\n", msg );
	    int bytes = nn_send(nano_socket, msg, strlen(msg), 0);
	    sleep(1);
	  } // end for testing
